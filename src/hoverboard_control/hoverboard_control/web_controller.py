...
<body>
  ...
  <div class="pad">
    ...
    <button onclick="move('F')">▲ Fwd</button>
    <div class="spacer"></div>
...
<script>
  const EMOTIONS = ['love','happy','smile','excited','laugh','kiss','wink',
                    'surprised','sad','confused','angry','sleepy','talking','neutral'];
  ...
  // Poll the robot's current mode once per second
  setInterval(() => {
    fetch('/mode').then(r => r.json()).then(d => {
      const manual = d.mode === 'MANUAL';
      banner.className = manual ? 'manual' : 'auto';
      banner.textContent = manual ? '🎮 Manual Control' : '🤖 Autonomous (' + d.mode + ')';
    }).catch(() => {});
  }, 1000);
</script>
</body>
...
def emotion(name):
    if node:
        ...
        node.get_logger().info(f'🎭 Emotion → {name}')
    return jsonify({'ok': True, 'emotion': name})
...
def move(cmd):
    if node:
        ...
        node.get_logger().info(f'🚗 Move → {cmd}')
    return jsonify({'ok': True, 'move': cmd})


@app.route('/mode')
def mode():
    return jsonify({'mode': node.current_state if node else 'UNKNOWN'})
...
class WebController(Node):
    def __init__(self):
        ...
        # Listen to the robot's mode so the phone can display it
        self.create_subscription(String, '/robot/state', self.state_callback, 10)
        self.get_logger().info('✅ Web controller ready → http://<robot-ip>:5000')
